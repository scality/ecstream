 ...
 *
 * @param {DecodeContext} decodeContext - Context of the stream decoding
 ...
 *
 * @param {DecodeContext} decodeCtx - Context of the stream decoding
 * @param {Buffer} dataBuffer - data part of the stripe
 * @param {Buffer} parityBuffer - parity part of the stripe
 ...
 * @return {undefined}
 */
function sendToEncoder(decodeContext) {
    decodeContext.ref();
    decodeStep(decodeContext,
 ...
 * @return {undefined}
 */
function decodeBufferStep(decodeContext, istreamId) {
    // Backpressure - avoids memory blow-up
    if (decodeContext.waitReading()) {
        return;
    }

    if (shouldSendToEncoder(decodeContext)) {
        sendToEncoder(decodeContext);
    }
 ...
 * @return {DecodeContext} decoding context
 *
...
function decode(ostream, size, dataStreams, parityStreams, stripeSize) {
    ...
    istreams.forEach((s, i) => {
        ...
        // Read handler
        s.on('readable', () => decodeBufferStep(decodeContext, i));
    });
